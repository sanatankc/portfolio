...
## Goose Library and Workflow Management


As our AI pipelines grew more complex, we quickly ran into the limits of off-the-shelf workflow tools. We needed something that could handle multi-step, branching processes, keep track of structured data at every stage, and let us experiment rapidly with new ideas.

That’s why I built **"Goose"**, a TypeScript library for defining event-driven, type-safe AI workflows. Here's how it looks:

```ts
const pipeline = goose<{ input: string; result?: string }>(
  { input: 'This is a really really long text, which keeps on going...' },
...
Read more about Goose [here](https://github.com/sanatankc/goose/blob/main/README.md).

On top of Goose, I built a Pipeline Dashboard for visualizing, resuming, and iterating on workflows. See it in action below:
...
<MediaGallery 
  ...
  items={[
    {
      type: "youtube",
      youtubeId: "xDbYCKKJ5dc",
      alt: "Example YouTube video"
    }
  ]} 
/>

This tooling enabled rapid iteration and async collaboration, letting the founder experiment with prompts and workflow changes while I focused on backend scaling and reliability.
...
- Finally, we applied a composite quality score to each project, evaluating them on key attributes like novelty, utility, entertainment, use of AI, and more.
...
With a curated list of around 5,000 high-quality projects, our next challenge was to enrich them with structured data.
...
We generated essential descriptive content to help users quickly understand each project. This included a project overview, a "why you'll love this" section, a "how it works" summary, and more. 
...
<MediaGallery 
  ...
  items={[
    {
      src: "/blog/videos/details-final.mp4",
      type: "video",
      alt: "Glitch.house platform overview"
    }
  ]} 
...
One of the most important outputs was a rich set of **tags**, we aimed for ~30 tags per project, covering everything from technologies used (like "CircuitPython" or "3D Printing") to project purpose ("Home Automation," "Wearable Tech"), theme (e.g., "Halloween"), materials, domain, and discipline. These tags became the backbone of our discovery and recommendation systems.


**2. The Component Extraction Pipeline:**  

Extracting a complete and accurate component list was challenging because components are often mentioned inconsistently across long project guides, and we did not want to miss anything, even small items like a paintbrush. To solve this, we designed a multi-layer pipeline:

- **Chunk by chunk:** We processed the source content chunk by chunk, feeding the list of components found in previous chunks as context to the next. This allowed the LLM to intelligently decide whether to merge a newly found part with an existing one (e.g., updating "Arduino" to "Arduino Uno") or add it as a new item.
- **Merging and cleaning:** After the initial pass, a separate layer deduplicated the list.
- **Adding details:** Another LLM workflow then took this clean list and, chunk by chunk, generated detailed information for each component.
...
We repeated this process, layer by layer, starting with the most common items. This allowed us to efficiently standardize the vast majority of our tags and components, creating a clean, reliable dataset for the platform.
...
We started by breaking down each project’s documentation into smaller content blocks and annotating it with metadata—keywords, content type (assembly, intro, promo, etc.), and author style (humor, directness, etc.). This let us track and organize even the messiest source material.
...
This multi-layered approach allowed us to generate build guides that were standardized, image-rich, and true to the original author’s intent, even when the source material was incomplete or inconsistent. It was by far the hardest part of the pipeline, but also the most satisfying to get right. This process took countless iterations to perfect.
...
Our recommender system connects users to the most relevant DIY projects based on their onboarding choices, such as interests, skill level, and available components, by matching these to the tags in our system.


When a user signs up, they select their themes, experience, and available hardware. We instantly match this profile by prioritizing projects that fit their skills and available components, but also nudging them toward slightly more challenging builds to encourage learning.
